  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-matte-800 pb-6">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-brand-950/30 text-brand-500 text-xs font-bold mb-2 border border-brand-900/50">
            <Calendar className="w-3.5 h-3.5" />
            Renter Dashboard
          </div>
          <h1 className="text-3xl font-extrabold text-white">My Rental Bookings ({myRentals.length})</h1>
      ...
      {myRentals.length > 0 ? (
        <div className="space-y-6">
          {myRentals.map(book => (
            <div 
              key={book.id}
              className="bg-matte-900 rounded-3xl p-6 sm:p-8 border border-matte-800 shadow-md hover:shadow-[0_0_15px_rgba(255,255,255,0.1)] hover:border-slate-400 transition-all flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6"
            >
              <div className="flex flex-col sm:flex-row items-start gap-5 flex-1">
                <img 
                  src={book.toolImage} 
                  alt={book.toolTitle} 
                  className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl object-cover shrink-0 border border-matte-800 shadow-md"
                />
                ...
                <div className="space-y-2 flex-1">
                  ...
                  <h3 
                    ...
                    className="text-lg sm:text-xl font-extrabold text-white hover:text-brand-500 cursor-pointer transition-colors flex items-center gap-1.5"
                  >
                    {book.toolTitle}
                    <ExternalLink className="w-4 h-4 text-slate-400 inline" />
                  </h3>

                  <div className="flex flex-wrap items-center gap-4 text-xs sm:text-sm text-slate-400 font-medium pt-1">
                    ...
                    <span>Owner: <strong className="text-white">{book.ownerName}</strong></span>
                  </div>

                  {book.message && (
                    <p className="text-xs text-slate-300 italic bg-matte-800 p-3 rounded-xl border border-matte-700 mt-2">
              ...
              <div className="bg-matte-800 p-5 rounded-2xl border border-matte-700 w-full lg:w-64 shrink-0 space-y-3 text-center sm:text-right flex flex-col justify-between">
                <div>
                  ...
                  {book.status === 'Pending' && (
                    <button
                      ...
                      className="mt-2 w-full py-1.5 px-3 bg-matte-900 hover:bg-matte-950 text-rose-500 border border-rose-900/50 hover:border-rose-800 font-bold rounded-xl text-[11px] transition-all cursor-pointer"
                    >
                      Cancel Request
                    </button>
                  )}
                ...
                <div className="pt-2 border-t border-matte-700 flex items-baseline justify-between sm:justify-end gap-2">
                  <span className="text-xs text-slate-400 font-semibold">Total Estimate:</span>
                  <span className="text-lg font-extrabold text-white">₹{book.totalEstimate}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="bg-matte-900 rounded-3xl border border-matte-800 p-12 text-center max-w-lg mx-auto my-12 space-y-4 shadow-soft">
          ...
          <h3 className="text-xl font-bold text-white">You have no rental requests</h3>
